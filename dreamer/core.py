...
class Dreamer:
    def __init__(self, observation_shape, action_dim, state_dim=30, rnn_hidden_dim=200, model_lr=6e-4, value_lr=8e-5, action_lr=8e-5, eps=1e-4, device='cpu'):
        channel = observation_shape[0]
        self.encoder = Encoder(channel).to(device)
        self.rssm = RecurrentStateSpaceModel(state_dim, action_dim, rnn_hidden_dim).to(device)
        self.obs_model = ObservationModel(state_dim, rnn_hidden_dim, channel).to(device)
    ...
    def update_model(self, observations: np.ndarray, actions: np.ndarray, rewards: np.ndarray, free_nats=3, clip_grad_norm=100):
        ...
        embedded_observations = self.encoder(observations.reshape(-1, *self.observation_shape)).view(chunk_length, batch_size, -1)
        ...
        recon_observations = self.obs_model(flatten_states, flatten_rnn_hiddens).view(chunk_length-1, batch_size, *self.observation_shape)
