...
class FixedGaussian(th.jit.ScriptModule):
    """
    ...
    def forward(self, x):
        mean = th.zeros(x.size(0), self.output_dim, device=x.device)
        std = th.ones(x.size(0), self.output_dim, device=x.device).mul_(self.std)
        return mean, std


class Gaussian(th.jit.ScriptModule):
    """
    ...
    def forward(self, x):
        if x.ndim == 3:
            ...
            x = self.net(x)
        mean, std = th.chunk(x, 2, dim=-1)
        std = F.softplus(std) + 1e-5
        return mean, std


class Decoder(th.jit.ScriptModule):
    """
    Decoder.
    """
    ...
    def forward(self, x):
        ...
        _, C, W, H = x.size()
        x = x.view(B, S, C, W, H)
        return x, th.ones_like(x).mul_(self.std)


class Encoder(th.jit.ScriptModule):
    """
    Encoder.
    """
...
class LatentModel(th.jit.ScriptModule):
    """
    ...
    def sample_prior(self, actions_, z2_post_):
        ...
        # p(z1(t) | z2(t-1), a(t-1))
        z1_mean_, z1_std_ = self.z1_prior(th.cat([z2_post_[:, : actions_.size(1)], actions_], dim=-1))
        # Concatenate initial and consecutive latent variables
        z1_mean_ = th.cat([z1_mean_init.unsqueeze(1), z1_mean_], dim=1)
        z1_std_ = th.cat([z1_std_init.unsqueeze(1), z1_std_], dim=1)
    ...
    def sample_posterior(self, features_, actions_):
        ...
        z1 = z1_mean + th.randn_like(z1_std) * z1_std
        # p(z2(0) | z1(0))
        z2_mean, z2_std = self.z2_posterior_init(z1)
        z2 = z2_mean + th.randn_like(z2_std) * z2_std
        ...
        for t in range(1, actions_.size(1) + 1):
            ...
            z1_mean, z1_std = self.z1_posterior(th.cat([features_[:, t], z2, actions_[:, t - 1]], dim=1))
            z1 = z1_mean + th.randn_like(z1_std) * z1_std
            # q(z2(t) | z1(t), z2(t-1), a(t-1))
            z2_mean, z2_std = self.z2_posterior(th.cat([z1, z2, actions_[:, t - 1]], dim=1))
            z2 = z2_mean + th.randn_like(z2_std) * z2_std
        ...
        z1_mean_ = th.stack(z1_mean_, dim=1)
        z1_std_ = th.stack(z1_std_, dim=1)
        z1_ = th.stack(z1_, dim=1)
        z2_ = th.stack(z2_, dim=1)
        return (z1_mean_, z1_std_, z1_, z2_)

    @th.jit.script_method
    def calculate_loss(self, state_, action_, reward_, done_) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        x = th.cat([z_[:, :-1], action_, z_[:, 1:]], dim=-1)
        B, S, X = x.shape
