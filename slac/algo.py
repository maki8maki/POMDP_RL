...
class SlacAlgorithm:
    ...
    def __init__(
        ...
        fake_feature = th.empty(1, num_sequences + 1, feature_dim, device=device)
        fake_action = th.empty(1, num_sequences, action_shape[0], device=device)
        self.create_feature_actions = th.jit.trace(create_feature_actions, (fake_feature, fake_action))

    def preprocess(self, ob: SlacObservation):
    ...
    def explore(self, ob: SlacObservation):
        feature_action = self.preprocess(ob)
        with th.no_grad():
            action = self.actor.sample(feature_action)[0]
        return action.cpu().numpy()[0]

    def exploit(self, ob: SlacObservation):
        feature_action = self.preprocess(ob)
        with th.no_grad():
            action = self.actor(feature_action)
        return action.cpu().numpy()[0]

    def step(self, env: gym.Env, ob: SlacObservation, is_random: bool):
        if is_random:
    ...
    def update_latent(self, writer: SummaryWriter):
        ...
        writer.add_scalar("train/latent_loss", loss_latent.item(), self.learning_steps_latent)
        writer.add_scalar("train/kld_loss", loss_kld.item(), self.learning_steps_latent)
        writer.add_scalar("train/reward_loss", loss_reward.item(), self.learning_steps_latent)
        writer.add_scalar("train/image_loss", loss_image.item(), self.learning_steps_latent)
    ...
    def update_critic(self, z, next_z, action, next_feature_action, reward, done, writer: SummaryWriter):
        ...
        assert isinstance(loss_critic, th.Tensor)

        self.optim_critic.zero_grad()
        loss_critic.backward(retain_graph=False)
        self.optim_critic.step()

        writer.add_scalar("train/critic_loss", loss_critic.item(), self.learning_steps_sac)

    def update_actor(self, z, feature_action, writer: SummaryWriter):
        ...
        writer.add_scalar("train/actor_loss", loss_actor.item(), self.learning_steps_sac)
        writer.add_scalar("train/alpha_loss", loss_alpha.item(), self.learning_steps_sac)
        writer.add_scalar("train/alpha", self.alpha.item(), self.learning_steps_sac)
        writer.add_scalar("train/entropy", entropy.item(), self.learning_steps_sac)
