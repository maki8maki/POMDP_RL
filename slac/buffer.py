...
class SequenceBuffer:
    ...
    def get(self):
        state_ = np.array(self.state_, dtype=np.uint8)
...
class ReplayBuffer:
    ...
    def __init__(self, buffer_size, num_sequences, state_shape, action_shape, device):
        ...
        self.state_ = np.empty((buffer_size, num_sequences+1, *state_shape), dtype=np.uint8)
        # Store other data on GPU to reduce workloads.
        self.action_ = th.empty(buffer_size, num_sequences, *action_shape, device=device)
        self.reward_ = th.empty(buffer_size, num_sequences, 1, device=device)
        self.done_ = th.empty(buffer_size, num_sequences, 1, device=device)
    ...
    def _append(self, state_, action_, reward_, done_):
        ...
        self.action_[self._p].copy_(th.from_numpy(action_))
        self.reward_[self._p].copy_(th.from_numpy(reward_))
        self.done_[self._p].copy_(th.from_numpy(done_))
    ...
    def sample_latent(self, batch_size):
        ...
        state_ = th.tensor(self.state_[idxes], dtype=th.uint8, device=self.device).float().div_(255.0)
